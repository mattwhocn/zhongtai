...
// techData
const defaultTechData: TechMeta[] = [
  {
    id: 0,
    type: "核心产品",
...
const TechBusiness: React.FC = () => {
  usePageTitle('科技板块');
  const [techData, setTechData] = useState<TechMeta[]>(defaultTechData);

  useEffect(() => {
    const fetchBannerConfig = async () => {
      try {
        const response = await axios.get('http://static.ztmagroup.com/data/json/tech/tech.json');
        // 将带有 \r\n 的字符串分割成数组
        const techData = response.data.map((item: any) => ({
          ...item,
          id: Math.random(),
        ...
        console.error('获取tech data配置失败:', error);
      }
    };
    fetchBannerConfig();
  }, []);
  ...
  const [coreProductsData, customCases] = useMemo(() => {
    const coreProductsData = techData.filter(item => item.type === '核心产品') ?? [];
    const customCases = techData.filter(item => item.type === '定制产品') ?? [];
